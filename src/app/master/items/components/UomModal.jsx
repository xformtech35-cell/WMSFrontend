...
export default function UomModal({ isOpen = false, onClose, onUomChange }) {
  ...
  const handleEditClick = (uom) => {
    ...
    setFormData({
      ...
      active: uom.active !== undefined ? uom.active : true,
    });
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      resetForm();
      loadUoms();
      if (onUomChange) onUomChange();
    } catch (error) {
      console.error("Error saving UOM:", error);
      setErrorMessage(
        error.message || `Failed to ${isEditing ? "update" : "create"} UOM.`
      );
    } finally {
      setSaving(false);
    }
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
              <h3 className="text-sm font-semibold text-gray-800 mb-3 flex items-center gap-2">
                {isEditing ? (
                  <>
                    <Edit className="w-4 h-4 text-green-600" /> Edit UOM
                  </>
                ) : (
                  <>
                    <Plus className="w-4 h-4 text-blue-600" /> Add New UOM
                  </>
                )}
              </h3>
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="submit"
                      disabled={saving}
                      className="px-4 py-1.5 text-xs font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-1 disabled:opacity-50"
                    >
                      {saving ? (
                        <>
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  className="pl-9 pr-3 py-1.5 w-full text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
              </div>
              <button
                type="button"
              ...
              <table className="w-full text-sm text-left text-gray-600">
                ...
                <tbody className="divide-y divide-gray-200">
                  {loading ? (
                    ...
                    uoms.map((uom) => (
                      <tr key={uom.id || uom.code} className="hover:bg-gray-50">
                        <td className="px-4 py-2.5 font-medium text-gray-900">
                          {uom.code}
                        </td>
                        <td className="px-4 py-2.5">{uom.name}</td>
