...
export default function ComparisonPopup({ data, onClose, onConvertToPO }) {
  ...
  const avgPrice =
    data.reduce((sum, q) => sum + (q.grandTotal || 0), 0) / data.length;
          ...
          <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center z-10">
            <div>
              ...
              <p className="text-sm text-gray-500">
                {data.length} quotations compared · Best quote:{" "}
              ...
              {sortedData.map((quotation, index) => {
                ...
                return (
                  ...
                  >
                    {/* Header */}
                    <div
                      className={`px-4 py-3 border-b ${isBest ? "border-green-200" : "border-gray-200"} flex justify-between items-center`}
                    >
                      ...
                      {commonItems.map((item, idx) => (
                        <tr key={idx} className="hover:bg-gray-50">
                          ...
                          {item.data.map((supplierData, sIdx) => {
                            ...
                            const isMin =
                              supplierData.price === minPrice && minPrice > 0;
                            const isMax =
                              supplierData.price === maxPrice &&
                              maxPrice > 0 &&
                              minPrice !== maxPrice;
