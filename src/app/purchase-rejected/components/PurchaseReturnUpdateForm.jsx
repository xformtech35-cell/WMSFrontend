...
export default function PurchaseReturnUpdateForm({ 
    ...
    try {
      ...
      if (data) {
        setFormData({
          ...
          requestDate: data.returnDate ? formatDateForInput(data.returnDate) : "",
  ...
  const calculateTotalAmount = () => {
    return formData.lines.reduce((sum, line) => sum + (Number(line.totalAmount) || 0), 0);
  };
    ...
    try {
      ...
      const vendorRequestData = {
        ...
        lines: formData.lines.map(line => ({
          ...
          unitPrice: Number(line.unitPrice) || 0,
          totalAmount: Number(line.totalAmount) || 0,
          originalQuantity: Number(line.originalQuantity) || Number(line.returnQuantity),
      ...
      setSuccessMessage(`Vendor return request sent successfully! Reference: ${response?.requestNumber || 'N/A'}`);
      setShowSuccess(true);
          ...
          <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4 flex justify-between items-center z-10">
            <div>
              ...
              <p className="text-green-100 text-sm">
                ...
                <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(formData.status)}`}>
                  {formData.status || "N/A"}
                </span>
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              handleLineChange(index, "returnQuantity", Number(e.target.value));
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {formData.approvedBy && (
                      <div>
                        ...
                        <p className="text-sm text-gray-900">{formData.approvedBy}</p>
                      </div>
                    )}
                    {formData.approvedDate && (
                      <div>
                        <p className="text-xs text-gray-500">Approved Date</p>
                        <p className="text-sm text-gray-900">{formatDate(formData.approvedDate)}</p>
                      </div>
                    )}
              ...
              <div className="flex justify-end gap-3 border-t border-gray-200 pt-4">
               ...
               <button
                    ...
                    title={formData.status !== "PENDING" ? "Only PENDING returns can be sent to vendor" : ""}
                  >
